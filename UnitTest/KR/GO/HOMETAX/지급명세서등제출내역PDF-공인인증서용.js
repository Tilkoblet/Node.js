const FS = require("fs");
const Rest = require("../../../../Tilko.API/REST").Tilko.API.REST;
const Constant = require("../../../../UnitTest/Constant").UnitTest.Constant;

var Tilko = Tilko || {};

(function (_Tilko) {

    // API 상세설명 URL
    // https://tilko.net/Help/Api/POST-api-apiVersion-HometaxGeneral-ATESFZAA001R04-MyeongSeSeo-Pdf
    
    try {
        let _rest = new Rest(Constant.ApiKey);
        _rest.Init();

        // 지급명세서등 제출내역 PDF endPoint 설정
        _rest.SetEndPointUrl(Constant.ApiHost + "api/v1.0/HometaxGeneral/ATESFZAA001R04-MyeongSeSeo-Pdf");

        // 공동인증서 경로 설정
        const _publicPath = Constant.CertPath + "/signCert.der";
        const _privatePath = Constant.CertPath + "/signPri.key";
        let _publicCert = Buffer.alloc(0);
        let _privateKey = Buffer.alloc(0);

        _publicCert = FS.readFileSync(_publicPath);
        _privateKey = FS.readFileSync(_privatePath);
        console.log("_publicCert:", _publicCert);
        console.log("_privateKey:", _privateKey);
        
        // Body 추가
        _rest.AddBody("CertFile", _publicCert, true);                   // [암호화] 인증서 공개키(Base64 인코딩)
        _rest.AddBody("KeyFile", _privateKey, true);                    // [암호화] 인증서 개인키(Base64 인코딩)
        _rest.AddBody("CertPassword", Constant.CertPassword, true);     // [암호화] 인증서 암호(Base64 인코딩)
        _rest.AddBody("CvaId", "", false);                              // CvaId (지급명세서등 제출내역 리스트로부터 획득 가능)
        _rest.AddBody("IeTin", "", false);                              // IeTin (지급명세서등 제출내역 리스트로부터 획득 가능)
        _rest.AddBody("LvyRperTin", "", false);                         // LvyRperTin (지급명세서등 제출내역 리스트로부터 획득 가능)
        _rest.AddBody("MateKndNm", "", false);                          // MateKndNm (지급명세서등 제출내역 리스트로부터 획득 가능)
        
        // API 호출
        const Response = _rest.Call();
        console.log("Response:", Response);
        console.log(Response.PdfData);
        FS.writeFileSync('D:/Temp/dd.pdf', Buffer.from(Response.PdfData, "base64"));
    } catch (e) {
        console.error(e);
    }

})(Tilko);
