...
(function (_Tilko) {
    ...
    // API 상세설명 URL
    // https://tilko.net/Help/Api/POST-api-apiVersion-HometaxSimpleAuth-ATESFZAA001R04-MyeongSeSeo-Pdf
    
    try {
        let _rest = new Rest(Constant.ApiKey);
        _rest.Init();

        // 지급명세서등 제출내역 PDF endPoint 설정
        _rest.SetEndPointUrl(Constant.ApiHost + "api/v1.0/HometaxSimpleAuth/ATESFZAA001R04-MyeongSeSeo-Pdf");
        ...
        _rest.AddBody("CvaId", "", false);                              // CvaId (지급명세서등 제출내역 리스트로부터 획득 가능)
        _rest.AddBody("IeTin", "", false);                              // IeTin (지급명세서등 제출내역 리스트로부터 획득 가능)
        _rest.AddBody("LvyRperTin", "", false);                         // LvyRperTin (지급명세서등 제출내역 리스트로부터 획득 가능)
        _rest.AddBody("MateKndNm", "", false);                          // MateKndNm (지급명세서등 제출내역 리스트로부터 획득 가능)
        
        // API 호출
        const Response = _rest.Call();
        console.log("Response:", JSON.stringify(Response));
        console.log(Response.JsonData);
        FS.writeFileSync('D:/Temp/지급명세서등제출내역.pdf', Buffer.from(Response.PdfData, "base64"));
    } catch (e) {
        console.error(e);
    }
